...
printf "(cd /scratch/$USER/cgenie.muffin-$i/genie-main; make cleanall; LD_LIBRARY_PATH=/scratch/dn3g22/cgenie.muffin-$i/netcdf_libs/lib; export LD_LIBRARY_PATH; ./runmuffin.sh $line $2/$line ${line}-${iteration}.config $3 &> ~/cgenie_log/muffin-basket-$(date '+%F_%H.%M')-${line}-${iteration}.log) &
"  >> ~/cgenie.jobs/muffin-basket-$short_name-$iteration.sbatch
else # wait 6 mins (recall five occassionally not being quite enough) for second 20 runs
j=$((i-20))
printf "(cd /scratch/$USER/cgenie.muffin-$j/genie-main; sleep 360; make cleanall; LD_LIBRARY_PATH=/scratch/dn3g22/cgenie.muffin-$j/netcdf_libs/lib; export LD_LIBRARY_PATH; ./runmuffin.sh $line $2/$line ${line}-${iteration}.config $3 &> ~/cgenie_log/muffin-basket-$(date '+%F_%H.%M')-${line}-${iteration}.log) &
"  >> ~/cgenie.jobs/muffin-basket-$short_name-$iteration.sbatch
fi
else # subsequent experiments all start from a restart
printf "(cd /scratch/$USER/cgenie.muffin-$i/genie-main; make cleanall; LD_LIBRARY_PATH=/scratch/dn3g22/cgenie.muffin-$i/netcdf_libs/lib; export LD_LIBRARY_PATH; ./runmuffin.sh $line $2/$line ${line}-${iteration}.config $3 ${line}-$((iteration - 1)).config &> ~/cgenie_log/muffin-basket-$(date '+%F_%H.%M')-${line}-${iteration}.log) &
"  >> ~/cgenie.jobs/muffin-basket-$short_name-$iteration.sbatch
fi
i=$((i+1))
done
...
ready_clones=$(find /scratch/dn3g22 -mindepth 2 -maxdepth 2 -type d -name "*genie-main" -mmin +2| wc -l)
